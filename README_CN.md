...
- 双主题支持：Claude（暖色浅色）和 Dark（奢华金色暗色），一键切换（v0.4.0 新增）
...
3. 注意地址末尾不要加 `/`，示例: `https://your-worker-name.your-subdomain.workers.dev/{PATH_SECRET}`
4. 如果你使用 Cloudflare Worker 后端，油猴脚本只需要填写地址和 `Transport Secret`。发送、接收、云端列表、云端删除都会使用同一个 `TRANSPORT_SECRET`
5. 管理面板访问 `https://your-worker-name.your-subdomain.workers.dev/{PATH_SECRET}/admin` 使用管理密码 `ADMIN_PASSWORD` 登入

### 后端部署指南

#### 选项 1: Cloudflare Worker + D1 (推荐)

> **⚠️ 注意：v0.4.1 版本后使用 D1 数据库，不兼容旧版（KV）数据。如果你是从旧版本升级，需要重新部署并迁移数据。**
